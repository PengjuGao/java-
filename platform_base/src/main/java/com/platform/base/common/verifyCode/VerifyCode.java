package com.platform.base.common.verifyCode;
...
/**
 * 验证码生成器类，可生成数字大写、小写字母及三混合类型的验证码
 * 支持自定义验证码字符数量
 * 支持自定义验证码图片的大小；
 * 支持自定义需排除的特殊字符；
 * 支持自定义干扰线的数量；
...
public class VerifyCode {
	 ...
	 * @param exChars 除的特殊字符（仅对数字字母混合型验证码有效，无除则为null
	 * @return 验证码字符串
	 */
	 ...
	 * 生成图片验证
	 * @param type 验证码类型，参见本类的静态属
	 * @param length 验证码字符长度，大于0的整
	 * @param exChars 除的特殊字符
	 * @param width 图片宽度
	 * @param height 图片高度
	 * @param interLine 图片中干扰线的条
	 * @param randomLocation 每个字符的高低位置是否随
